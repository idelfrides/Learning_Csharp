...
class ManageFiles
{
    ...
    {
        public static async Task MainTX(string[] args)
        {
           ...
           iomo.ShowPrettyInfo(crud_info, lineShape, 60);
            ...
            {
                Console.WriteLine($"\n\n\t Você não digitou um nome para o arquivo, então o nome do arquivo será o nome padrão: <{tfm.FullFilePath}>");
            } 
            else
            {
                tfm.FullFilePath = $"STAGE/{inputFileName}.txt";
            }
            ...
            Console.Write($"\n\n\t Digite o CONTEÚDO o arquivo <{tfm.FullFilePath}> ::>>  ");
            var fileContent = Console.ReadLine() is string s && s.Length > 0 ? s : lorenIpsumText;

            File.WriteAllText(tfm.FullFilePath, fileContent);

            // READ: Read the contents of the file
            infos = $"READ: Read the contents of the file <{tfm.FullFilePath}>";
            iomo.ShowPrettyInfo(infos, lineShape, 60);
            string readText = File.ReadAllText(tfm.FullFilePath);
            Console.WriteLine(readText);   // Output the content

            // UPDATE : Add a new content to end of the file
            infos = $"UPDATE : Add a new content to end of the file <{tfm.FullFilePath}>";
            iomo.ShowPrettyInfo(infos, lineShape, 60);

            Console.Write($"\n\n\t Digite outro CONTEÚDO PARA arquivo <{tfm.FullFilePath}> ::>>  ");
            inputContent = Console.ReadLine();

            var fileContent2 = string.IsNullOrEmpty(inputContent) ? whyDoWeUseIt : inputContent;

            File.AppendAllText(tfm.FullFilePath, fileContent2);
            Console.Write($"\n\n\tCONTEÚDO do arquivo <{tfm.FullFilePath}> ATUALIZADO \n\n");
            readText = File.ReadAllText(tfm.FullFilePath);
            Console.WriteLine(readText);   // Output the content
